...
public:
    ...
    wxBitmap(const void* data, wxBitmapType type, int width, int height, int depth = 1, bool rgba = false);
    ...
    wxBitmap(const wxImage& image, int depth = -1, double scale = 1.0, bool rgba = false);
    wxBitmap(const wxImage& image, const wxDC& dc, bool rgba = false);
...
private:
    void InitFromImage(const wxImage& image, int depth, double scale, bool rgba);
};
