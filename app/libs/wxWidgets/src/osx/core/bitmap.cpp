...
public:
    wxBitmapRefData(int width , int height , int depth, double logicalscale = 1.0, bool rgba = false);
...
private :
    bool Create(int width , int height , int depth, double logicalscale, bool rgba = false);
...
wxBitmapRefData::wxBitmapRefData( int w , int h , int d , double logicalscale, bool rgba )
{
    Init() ;
    Create( w , h , d, logicalscale, rgba ) ;
}
...
bool wxBitmapRefData::Create(int w, int h, int WXUNUSED(d), double logicalscale, bool rgba)
{
...
//     ARGB32
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaFirst) | kCGBitmapByteOrder32Host;

    // RGB32
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaNoneSkipFirst) | kCGBitmapByteOrder32Host;

    // RGBA8888_Premultiplied
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaPremultipliedLast) | kCGBitmapByteOrder32Big;

    // RGBX8888
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaNoneSkipFirst) | kCGBitmapByteOrder32Big;
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaPremultipliedLast);// | kCGBitmapByteOrder32Big;
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaNoneSkipLast) | kCGBitmapByteOrder32Little;

    // RGBA8888
    if ( rgba )
    {
        auto bitmapInfo = CGBitmapInfo(kCGImageAlphaPremultipliedFirst) | kCGBitmapByteOrder32Big;// | kCGBitmapByteOrder32Big;
        m_hBitmap = CGBitmapContextCreate(NULL, width, height, 8, 0, wxMacGetGenericRGBColorSpace(), bitmapInfo );
    }
    else
    {
...
//    m_hBitmap = CGBitmapContextCreate(NULL, width, height, 8, bytesPerRow, wxMacGetGenericRGBColorSpace(), rgba ? kCGImageAlphaPremultipliedFirst : kCGImageAlphaNoneSkipFirst );


    // ARGB_Premultiplied
//    auto bitmapInfo = CGBitmapInfo(kCGImageAlphaPremultipliedLast) | kCGBitmapByteOrder32Host;
...
//    CGContextRef hBitmap = CGBitmapContextCreate(NULL, GetWidth(), GetHeight(), 8, GetBytesPerRow(), wxMacGetGenericRGBColorSpace(), use ? kCGImageAlphaPremultipliedLast : kCGImageAlphaNoneSkipFirst );
...
CGImageRef CreateTintedImage(CGImageRef image)
{
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    size_t rowBytes = 4 * width;

    unsigned char* data = (unsigned char*)calloc(height * rowBytes, sizeof(unsigned char));

    CGColorSpaceRef colorspace = wxMacGetGenericRGBColorSpace();

    CGContextRef context = CGBitmapContextCreate(data, width, height, 8, rowBytes, colorspace, kCGImageAlphaPremultipliedLast | kCGBitmapByteOrderDefault /*kCGImageAlphaNone*/);
...
//    CGContextSetBlendMode(context, kCGBlendModeNormal);
    ...
    unsigned char* dst = (unsigned char*)calloc(height * rowBytes, sizeof(unsigned char));

    for (int i = 0; i < height * rowBytes; )
    {
        dst[i] = ((CGFloat) data[i + 3]);
        dst[i + 1] =   ((CGFloat) data[i]    );
        dst[i + 2] = ((CGFloat) data[i + 1]);
        dst[i + 3] =  ((CGFloat) data[i + 2]);
        i+=4;
    }

    CGContextRelease(context);
    context = CGBitmapContextCreate(dst, width, height, 8, rowBytes, colorspace, kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault /*kCGImageAlphaNone*/);
...
//    const vImage_Buffer src = { data, height, width, rowBytes};
//    const vImage_Buffer dst = { data, height, width, rowBytes};
...
//    image = CGImageCreate(width, height, 8, 32, rowBytes, colorspace, info | kCGBitmapByteOrder32Host, provider, nullptr, false,
//                          kCGRenderingIntentDefault);
//    CGDataProviderRelease(provider);
//

//    context = CGBitmapContextCreate(dst.data, width, height, 8, rowBytes, colorspace, kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault /*kCGImageAlphaNone*/);
...
CGImageRef wxBitmapRefData::CreateCGImage() const
{
    ...
    if ( m_rawAccessCount > 0 || m_cgImageRef == NULL )
    {
        if ( m_hBitmap )
        {
            image = CreateTintedImage(CGBitmapContextCreateImage(m_hBitmap));
//            image = CGBitmapContextCreateImage(m_hBitmap);
    ...
    {
        image = m_cgImageRef;
//        image = CreateTintedImage(m_cgImageRef);
        CGImageRetain( image ) ;
    }
...
wxBitmap::wxBitmap(const void* data, wxBitmapType type, int width, int height, int depth, bool rgba)
{
...
void wxBitmap::InitFromImage(const wxImage& image, int depth, double scale, bool rgba)
{
    ...
    m_refData = bitmapRefData = new wxBitmapRefData( width, height, depth, scale, rgba) ;
...
wxBitmap::wxBitmap(const wxImage& image, int depth, double scale, bool rgba)
{
    InitFromImage(image, depth, scale, rgba);
}

wxBitmap::wxBitmap(const wxImage& image, const wxDC& dc, bool rgba)
{
    InitFromImage(image, -1, dc.GetContentScaleFactor(), rgba);
}
