                  ...
                  CGImageRef      inImage)
{
    ...
    auto imageBitmapInfo = CGImageGetBitmapInfo(inImage);
    auto wid = CGImageGetWidth(inImage);
    auto concat = CGBitmapInfo(kCGImageAlphaPremultipliedFirst);
    if (imageBitmapInfo == concat && wid > 1000)
    {
        CGContextSetBlendMode(inContext, kCGBlendModeCopy);
    }

    CGContextSetInterpolationQuality(inContext, kCGInterpolationNone);
...
//        wxOSXDrawNSImage( m_cgContext, &r, bmp.GetNSImage());
        wxMacDrawCGImage( m_cgContext, &r, bmp.CreateCGImage());
    }
#else
