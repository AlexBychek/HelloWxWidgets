...
//        self.layer.contents = (__bridge id)CGDisplayCreateImage(kCGDirectMainDisplay);
//        [self setWantsLayer:YES];
//
//        auto newLayer = [CALayer layer];
//        newLayer.frame = CGRectMake(0, 0, 500, 500);
//        newLayer.backgroundColor = [NSColor colorWithWhite:0.0 alpha:0.5].CGColor;
////        newLayer.contents = (__bridge id)CGDisplayCreateImage(kCGDirectMainDisplay);
...
void wxWidgetCocoaImpl::Activate()
{
    NSView* nsview = m_osxView;
...
bool wxWidgetCocoaImpl::SetFocus()
{
    ...
    if ( [targetView canBecomeKeyView] )
    {
        ...
        [[m_osxView window] makeKeyWindow] ;
    }
...
wxWidgetImpl* wxWidgetImpl::CreateContentView( wxNonOwnedWindow* now )
{
    ...
    WXWidget ww = c->GetWXWidget();
    [[ww window] setColorSpace:[[[NSColorSpace alloc] initWithCGColorSpace:wxMacGetGenericRGBColorSpace()] autorelease]];
    return c;
}
