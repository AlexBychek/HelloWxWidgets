...
//    [m_macWindow orderFront:m_macWindow];

//    [m_macWindow makeFirstResponder:m_macWindow];
//    [m_macWindow makeKeyWindow];
}

void wxNonOwnedWindowCocoaImpl::Activate()
{
    [m_macWindow orderFrontRegardless];
    [NSApp activateIgnoringOtherApps:YES];
}
...
//    m_macWindow.contentView.layer.contents = (__bridge id)cgimage;
    wxNonOwnedWindow* wxpeer = GetWXPeer();
    if ( wxpeer )
    {
        NSView * parentView = GetWXPeer()->GetPeer()->GetWXWidget();
        if ( parentView )
        {
            parentView.layer.backgroundColor = [NSColor colorWithWhite:0.0 alpha:0.5].CGColor;
        }
    }
    if (m_macWindow.contentView.layer)
    {
        int a;
    }
    else
    {
//        m_macWindow.contentView.layer = CALayer();
    }
//    m_macWindow.contentView.layer.backgroundColor = [NSColor colorWithWhite:0.0 alpha:0.5].CGColor;
//    [m_macWindow setBackgroundColor:[NSColor colorWithWhite:0.0 alpha:0.5]];
//    [m_macWindow setBackgroundColor:col.OSXGetNSColor()];
#endif
   return true;
}
...
void wxNonOwnedWindowCocoaImpl::SetFloatView()
{
    [m_macWindow setLevel:NSStatusWindowLevel];
}
...
bool wxNonOwnedWindowCocoaImpl::EnableFullScreenView(bool enable, long style)
{
    ...
    {
#if 0
//        [m_macWindow setLevel:NSMainMenuWindowLevel + 1];
        [m_macWindow setTitleVisibility:NSWindowTitleHidden];
        [m_macWindow setReleasedWhenClosed:YES];
        [m_macWindow setHidesOnDeactivate:NO];
        [m_macWindow setStyleMask:NSBorderlessWindowMask | NSNonactivatingPanelMask];
//        [m_macWindow setLevel:kCGMainMenuWindowLevel - 1];
                [m_macWindow setLevel:NSMainMenuWindowLevel + 1];
        [m_macWindow setCollectionBehavior: NSWindowCollectionBehaviorCanJoinAllSpaces|NSWindowCollectionBehaviorFullScreenAuxiliary];

        [m_macWindow center];
        [m_macWindow orderFront:nil];
#else
        [m_macWindow setStyleMask:NSBorderlessWindowMask];
        [m_macWindow setCollectionBehavior: NSWindowCollectionBehaviorCanJoinAllSpaces|NSWindowCollectionBehaviorFullScreenAuxiliary];
        [m_macWindow setLevel:NSStatusWindowLevel];
        [m_macWindow setHidesOnDeactivate:NO];
#endif
