...
bool wxGenericAboutDialog::Create(const wxAboutDialogInfo& info, wxWindow* parent)
{
    ...
    SetFont({ wxFontInfo(10.5).FaceName(wxT("Montserrat")).Weight(wxFONTWEIGHT_MEDIUM) });
    ...
    copyrightSizer->Add(new wxHyperlinkCtrl(this, wxID_ANY,
        info.GetWebSiteDescription(),
        info.GetWebSiteURL()));

    auto copyrightText = new wxStaticText(this, wxID_ANY, info.GetCopyrightToDisplay());
    copyrightText->SetForegroundColour(*wxBLACK);
    copyrightSizer->Add(copyrightText);

    //////////////////////

    m_sizerText->Add(copyrightSizer, wxSizerFlags().Border(wxDOWN).Centre());
...
void wxGenericAboutDialog::AddTopControl(wxWindow* win, const wxSizerFlags& flags)
{
    wxCHECK_RET(m_sizerText, wxT("can only be called after Create()"));
    wxASSERT_MSG(win, wxT("can't add NULL window to about dialog"));

    sizerTop->Add(win, flags);
    Fit();
}

void wxGenericAboutDialog::AddTopControl(wxWindow* win)
{
    AddTopControl(win, wxSizerFlags().Border(wxDOWN).Centre());
}
...
void wxGenericAboutDialog::AddText(const wxString& text)
{
    if ( !text.empty() )
    {
        auto textControl = new wxStaticText(this, wxID_ANY, text);
        textControl->SetForegroundColour(*wxBLACK);
